...
class SignUp(generics.CreateAPIView):
    ...
    def post(request):
        ...
        if not username:
                return Response(Constants['error'].USERNAME_REQUIRED, status=HTTP_400_BAD_REQUEST)

        if username and password:

            if User.objects.filter(username=username).first():
                return HttpResponse(Constants.ERROR_USER_EXISTS, status=HTTP_400_BAD_REQUEST)
            ...
            created_user, created = User.objects.get_or_create(username=username)

            if created:
                save_user(request, created_user, password, email)

                async_to_sync(get_channel_layer().group_send)('users', {
                    'type': 'send.message',
                    'channel': 'users',
                })
...
def save_user(request, created_user, password, email):
    # set email
    if email:
    ...
    profile.full_name = created_user.username
    profile.save()

    # Authenticate user
    user = authenticate(username=created_user.username, password=password)
...
class Login(generics.CreateAPIView):
    ...
    def post(request):
        ...
        if username and password and len(username) and len(password):
            ...
            return Response(Constants.ERROR_ALL_FIELD_REQUIRED, status=404)
